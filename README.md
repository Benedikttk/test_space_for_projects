...
Let $s$ = sum with every ace counted as 11:

$$T(H) = s - 10 \cdot \max\!\left(0,\; \left\lceil \frac{s-21}{10} \right\rceil\right)$$

The hand is **soft** when at least one ace remains at 11 and $T \leq 21$:
...
$$P(D = d \mid u, \mathcal{S}, \neg\text{BJ}) = \frac{\displaystyle\sum_{h \neq f} \frac{n_h}{M} \cdot P(D=d \mid [u,h], \mathcal{S}^{-} \setminus \{h\})}{\displaystyle\sum_{h \neq f} \frac{n_h}{M}}$$
...
+\lambda & P = \text{BJ},\; d \neq 21 \\
+\lambda & P = \text{BJ},\; d = 21,\; \text{natural beats dealer 21} \\
0        & P = \text{BJ},\; d = 21,\; \text{natural does not beat dealer 21} \\
+1       & P > d \\
-1       & P < d \\
0        & P = d
\end{cases}$$

$\lambda$ = `blackjack_payout` (e.g. $1.5$ for 3:2) for naturals; $\lambda=1$ otherwise.
...
Classic example: hard 16 vs. dealer T where $\text{EV}_{\text{stand}} \approx -0.54$.
...
$$\text{EV}_{\text{insurance}} = 2 \cdot P(\text{hole} = T \mid \mathcal{S} \setminus \{A\}) - 1 = \frac{2 n_T}{M - 1} - 1$$

Insurance is $+\text{EV}$ when $P(\text{hole} = T) > \tfrac{1}{3}$, which
corresponds to a true count of approximately $+3$.

On a fresh 8-deck shoe: $P(T) = 128/415 \approx 0.308$, giving
$\text{EV} \approx -0.077$ (house edge).
...
$$\text{EV}_{\text{damp}} = \begin{cases}
\text{EV}_{\text{basic}} & \rho < \rho_{\min} \\
\text{EV}_{\text{basic}} + \sqrt{\dfrac{\rho - \rho_{\min}}{1 - \rho_{\min}}} \cdot (\text{EV}_{\text{raw}} - \text{EV}_{\text{basic}}) & \rho_{\min} \leq \rho < 1 \\
\text{EV}_{\text{raw}} & \rho \geq 1
\end{cases}$$
...
($\sigma^2 \approx 1.15$ for standard rules), the **Kelly fraction** of bankroll
to wager is:

$$f^{*} = \frac{\text{EV}}{\sigma^2}$$

The engine uses **half-Kelly** by default ($f = 0.5 \cdot f^{*}$) to reduce
variance while retaining ~75% of the long-run growth rate:

$$f_{\text{half}} = \frac{\text{EV}}{2\,\sigma^2}$$

The recommended bet is clamped to $[\text{min\_bet},\, \text{max\_bet}]$ and
rounded to the nearest min-bet increment. When $\text{EV} \leq 0$, bet the
table minimum.
...
is the **maximum-entropy prior**: uniform residual distribution is the
least-biased estimate in the absence of prior information. As
$|\mathcal{O}| \to 52N$ the estimate converges to the true posterior.
